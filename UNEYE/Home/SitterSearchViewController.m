...
#import "SitterSearchViewController.h"
...
@interface SitterSearchViewController ()<BasicSearchCellDelegate, AdvanceSearchBtnCellDelegate, ProfileSubmitBtnCellDelegate, CalenderPopUpViewControllerDelegate>{
...
@implementation SitterSearchViewController
...
//    [[CommonUtility sharedInstance] addScreenDataInfo:@"Babysitting" key:searchTypeofcare];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"SitterOptionViewController"])
    {
...
{    
    if (indexPath.row == 0)
    {
        ...
        MyIdentifier = @"basicSearchCell";
        BasicSearchCell *cell = (BasicSearchCell *)[tableView dequeueReusableCellWithIdentifier:MyIdentifier];
        
        if (cell == nil)
        {
            cell = [[BasicSearchCell alloc] initWithStyle:UITableViewCellStyleDefault
        ...
        if ([[[CommonUtility sharedInstance].screenDataDictionary valueForKey:searchSubTypeofcare] length] > 0) {
            cell.required.selectedItem = [[CommonUtility sharedInstance].screenDataDictionary valueForKey:searchSubTypeofcare];
        }
        
        if ([[[CommonUtility sharedInstance].screenDataDictionary valueForKey:searchPostCode] length] > 0) {
            cell.postCode.text = [[CommonUtility sharedInstance].screenDataDictionary valueForKey:searchPostCode];
        }
...
- (void)submitForm{
    ...
    if ([_txtSearch.text length] > 0) {
        ...
        if ([[CommonUtility sharedInstance].screenDataDictionary count] > 0) {
            ...
            if ([[[CommonUtility sharedInstance].screenDataDictionary valueForKey:@"field_sub_typeofcares"] length] > 0) {
                
                NSString *levelOfStudy = [[CommonUtility sharedInstance].screenDataDictionary valueForKey:@"field_sub_typeofcares"];
               
                [[CommonUtility sharedInstance].screenDataDictionary setValue:levelOfStudy forKey:searchLevelOfStudy];
                [[CommonUtility sharedInstance].screenDataDictionary removeObjectForKey:@"field_sub_typeofcares"];
...
-(NSMutableDictionary *)arrangeDateTimeParam:(NSMutableDictionary*)parameters{
    ...
    if ([[parameters valueForKey:searchDate] length] > 0) {
        ...
        if ([[parameters valueForKey:searchTime] length] > 0) {
            ...
            [parameters removeObjectForKey:searchDate];
        }
    }
    if ([[parameters valueForKey:searchSubTypeofcare] isEqualToString:@"Select"]) {
        [parameters removeObjectForKey:searchSubTypeofcare];
    }
...
-(void)searchUserByName:(NSString*)name {
    ...
    [[IQWebService service] searchByName:headers name:name completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error && statusCode == 200) {
            ...
            [self performSegueWithIdentifier:@"SitterOptionViewController" sender:nil];
